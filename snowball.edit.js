...
(function (window, $) {
	...
	/*			for(var i=0;i<css.length;i+=1){
				...
				if (InputOption.color_name) {
					//カラーネームの最適化
					for (var h in color) if (-1 !== b.indexOf(h.toLowerCase())||-1 !== b.indexOf(h.toUpperCase())) b=css.replace(b,RegExp("([: ,\)\(]|[\t ]?:[\t ]?)([^;\{\}]*?)" + h + "((?:!important)|[, )(;}\n\r]|$)", "gim"), "$1$2" + color[h] + "$3")
				}
		...
		$option_all.on("click", function (e) {
			...
			for (i = 0, j = $custom.length; i < j; i += 1) {
				if (toggle&&!$custom.eq(i).hasClass("checked")||!toggle&&$custom.eq(i).hasClass("checked")) $custom.eq(i).click()
			}
		});
