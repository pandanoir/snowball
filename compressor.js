...
(function(){
    ...
    var Compressors=window.Compressors={
        ...
        clearSpaceAroundComma:function(cssBlock){
            cssBlock.selector(types['OTHERS']).replace(/[\t ]*,[\t ]*/g, ","); //コンマ周辺のスペース
            cssBlock.selector(types['BLOCKS']).replace(/[\t ]*,[\t ]*/g, ","); //コンマ周辺のスペース
        },
        clearIndent:function(cssBlock){
            //インデント消去
            var blocks2=cssBlock.selector(types['NEWLINE']);
            for(var i=blocks2.length-1;i>=0;i--){
                loop:while(true){
                    var block2=cssBlock.getBlock()[blocks2[i][2]+1];//改行の後ろのブロック
                    if(block2===undefined) break loop;
                    if(block2[1]===types['TAB']){
                        cssBlock.deleteBlock(blocks2[i][2]+1);
                    }else{
                        block2[0]=block2[0].replace(/^[\t ]*/g, ""); //インデントを削除
                        cssBlock.setBlock(blocks2[i][2]+1,block2); //インデントを削除後、更新
        ...
        clearSpace:function(cssBlock,blocks,key){
            var blocks;
            for(var i=0,j=blocks.length;i<j;i++){
                if(blocks[i][2]-1>=0){
                    //直前の要素がある==blocks[i][2]-1が0以上
                    var n=1;
                    loop:while(true){
                        var block=cssBlock.getBlock()[blocks[i][2]-n];//コロンか波括弧の直前のブロック
                        if(block[1]===types['TAB']){
                            cssBlock.deleteBlock(blocks[i][2]-n);
                            //要素を削除したから、nを足さなくてOK
                        }else{
                            block[0]=block[0].replace(/[\t ]*$/g,"");
                            cssBlock.setBlock(blocks[i][2]-n,block); //コロンの前のもののスペースを消す
                            if(block[0].length!=0) break loop;
                            n++;
                        }
                    }
                }
                var blocks2=cssBlock.getBlock();
                var n=blocks[i][2]+1;
                if(blocks[i][2]+1<=blocks2.length-1){
                    //直後の要素がある==blocks[i][2]-1がblocks2の要素数-1以下
                    var n=1;
                    loop:while(true){
                        var block=cssBlock.getBlock()[blocks[i][2]+n];//コロンか波括弧の直前のブロック
                        if(!block) break loop;
        ...
        compress:function(option,b) {
            ...
            if(!option.option||option.indent) Compressors.clearIndent(css);
            if(!option.option||option.line) css.selector(types['NEWLINE']).del(); //改行
            if(!option.option||option.comma) Compressors.clearSpaceAroundComma(css);
            if(!option.option||option.colon) Compressors.clearSpace(css,css.selector(types['COLON']),'colon');//コロンを選択
            if(!option.option||option.sc_bracket) Compressors.clearSpace(css,css.selector(types['LEFT_CURLY_BRACKET']),'sc_bracket');//左波括弧を選択
            if(!option.option||option.ec_bracket) Compressors.clearSpace(css,css.selector(types['RIGHT_CURLY_BRACKET']),'ec_bracket');//右波括弧を選択
            ...
            if (option.color_name) Compressors.optimizeColorName(css);//カラーネームの最適化
            if (option.padding && -1 !== b.indexOf("padding")) Compressors.optimizePadding(css); //パディング最適化
            if (option.margin && -1 !== b.indexOf("margin")) Compressors.optimizeMargin(css); //マージン最適化
            if (option.color) css.selector(types['BLOCKS']).replace(/\#([0-9a-fA-F])\1([0-9a-fA-F])\2([0-9a-fA-F])\3/g, "#$1$2$3"); //カラーを6桁から3桁へ
            if (option.lower) Compressors.colorCodeToLowerCase(css);//カラーコードを小文字にする
            if (option.none) Compressors.noneToZero(css);//noneを0に変換する
            return css;
    }
};
})();
